...
def test_search_omdb_sucesso(client):
    mock_response = {
        "Response": "True",
        "Title": "Inception",
        "Year": "2010",
    ...
    with patch("app.routes.requests.get") as mock_get:
        ...
        res = client.get("/movies/search?q=Inception")
        assert res.status_code == 200
        data = res.get_json()
...
def test_search_omdb_nao_encontrado(client):
    mock_response = {"Response": "False", "Error": "Movie not found!"}
    with patch("app.routes.requests.get") as mock_get:
        mock_get.return_value.raise_for_status = lambda: None
        mock_get.return_value.json.return_value = mock_response

        res = client.get("/movies/search?q=xyzfilmeinexistente")
        assert res.status_code == 404

def test_search_omdb_sem_parametro(client):
